...
Reason about the user's objective, but never claim an external action happened unless the runtime actually executed and verified it.
"""

PLANNER_PROMPT = """You are Aibo's planning engine.
Return ONLY valid JSON with this shape:
{"steps":[{"id":"step-1","objective":"...","capability":"respond","agent":"general","payload":{"objective":"..."},"requires_approval":false}]}
...
class LLMClient:
    def _request(self, system: str, user: str) -> str:
        ...
        if provider not in {"openai", "openai_compatible"}: raise LLMError(f"Unsupported LLM provider: {settings.llm_provider}")
        if not settings.llm_api_key: raise LLMError("LLM API key is not configured")
    ...
    def plan(self, objective: str, runtime_contract: str | None = None) -> dict[str, Any]:
        provider = settings.llm_provider.lower().strip()
        if provider == "stub":
            return {"steps":[{"id":"step-1","objective":objective,"capability":"respond","agent":"general","payload":{"objective":objective},"requires_approval":False}]}
        user_prompt = objective
        if runtime_contract: user_prompt = f"{objective}\n\nRUNTIME CONTRACT:\n{runtime_contract}"
        ...
        try:
            ...
            if not isinstance(data, dict) or not isinstance(data.get("steps"), list) or not data["steps"]: raise ValueError("planner returned no steps")
            return data
