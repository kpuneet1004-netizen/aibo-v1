...
def create_mission(request:MissionCreate):
    ...
    mission.status=MissionStatus.RUNNING; mission_store.update(mission); worker.enqueue(task)
...
@router.get("/events")
def recent_events(limit:int=50): return event_bus.recent(max(1,min(limit,100)))

@router.get("/worker")
def worker_status(): return {"worker_id":worker.worker_id,"running":worker.running,"queue_size":task_queue.size()}
